...
static char **string_array_from_iter(DBusMessageIter iter, int array_len)
{
	...
	for (i = 0; dbus_message_iter_get_arg_type(&sub) != DBUS_TYPE_INVALID;
...
static void query_reply(DBusPendingCall *call, void *user_data)
{
	...
	while (dbus_message_iter_get_arg_type(&element) != DBUS_TYPE_INVALID) {
		...
		if (node != NULL)
			session->request->generate_response(
						(const char **) node, session);
...
int messages_get_messages_listing(void *s,
				const char *name,
				uint16_t max, uint16_t offset,
				...
				void *user_data)
{
	...
	if (query_tracker(query, session, &err) == NULL)
		g_free (request);
